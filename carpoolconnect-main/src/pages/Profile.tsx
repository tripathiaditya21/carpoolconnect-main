      ...
      try {
        ...
        if (userRequests && userRequests.length > 0) {
          ...
          for (const request of userRequests) {
            ...
            const matchingRide = rides.find(ride => ride.id === request.rideId);
            
            if (matchingRide) {
              // Add the request info to the ride
              requestedRides.push({
                ...matchingRide,
          ...
          // Fallback to random rides for demo purposes
          setRequestHistory(
            rides
        ...
        // Set the rides state
        setUpcomingRides(upcomingRides.slice(0, 3));
        setPastRides(pastRides.slice(0, 3));
        
        // Get user ride offers (mock data for now)
        setOfferHistory(rides.filter((_, index) => index % 5 === 0).slice(0, 2));
      } catch (error) {
        console.error('Error in loadRides:', error);
